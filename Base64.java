...
public class Base64
{
    ...
    /** Specify that data was compressed (value is <tt>true</tt>). */
    public final static boolean COMPRESSED = true;
    
    
    /** Specify that data was not compressed (value is <tt>false</tt>). */
    public final static boolean NOT_COMPRESSED = false;



    /** Break lines when encoding at 76-ish characters. */
    public final static boolean BREAK_LINES = true;


    /** Don't break lines when encoding (violates strict Base64 specification) */
    public final static boolean DONT_BREAK_LINES = false;
...
/* ********  E N C O D I N G   M E T H O D S  ******** */    
     ...
     * The array <var>threeBytes</var> needs only be as big as
     * <var>numSigBytes</var>.
     * You can reuse a byte array by passing a four-byte array as <var>b4</var>.
     *
     * @param threeBytes the array to convert
     * @param numSigBytes the number of significant bytes in your array
     * @return four byte array in Base64 notation.
    ...
    {
        return encodeObject( serializableObject, true, false );
    }   // end encodeObject
     ...
     * cannot be serialized or there is another error,
     * the method will return <tt>null</tt>.
     * You can specify whether or not to GZip-compress the object
    ...
    boolean gzipCompress )
    {
        ...
        java.util.zip.GZIPOutputStream gzout = null;
        ...
        try
        {
            ...
            if( gzipCompress )
            {
                gzout = new java.util.zip.GZIPOutputStream( b64os );
                oos   = new java.io.ObjectOutputStream( gzout );
            }   // end if: compress
            else
            {
    ...
    /**
     * Encodes a byte array into Base64 notation.
     * Optionally GZip-compresses data.
     *
     * @param source The data to convert
     * @param off Offset in array where conversion should begin
     * @param len Length of data to convert
     * @param breakLines Break lines at 80 characters or less.
     * @param gzipCompress Compress data before encoding it.
     * @since 1.5
     */
    public static String encodeBytes( byte[] source, int off, int len, boolean breakLines, boolean gzipCompress  )
    {
        if( !gzipCompress )
            return encodeBytes( source, off, len, breakLines );
        ...
        try
        {
            Base64.OutputStream b64os = new Base64.OutputStream( baos, ENCODE, breakLines );
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    /**
     * Decodes data from Base64 notation, optionally decompressing
     * the data after it is decoded. 
     *
     * @param s the string to decode
     * @param gzipCompressed Whether or not the data is compressed within
     * @since 1.5
     */
    public static byte[] decode( String s, boolean gzipCompressed )
    {
        // If it's not compressed, don't involve streams.
        if( !gzipCompressed )
            return decode( s );;
        
        // Thanks to Wentao Guo for fixing this code.
        java.io.InputStream is=null;
        java.io.ByteArrayOutputStream baos=null;
        byte[] buffer=new byte[2048];
        int length=0;
        try{
            is=new java.util.zip.GZIPInputStream(new java.io.ByteArrayInputStream(decode(s)));
            baos  = new java.io.ByteArrayOutputStream();
     ...
     * Assumes object within is not GZip-compressed.
     *
    ...
    {
        return decodeToObject( encodedObject, false );
    }   // end decodeToObject
     ...
     * Attempts to decode Base64 data and deserialize a Java
     * Object within. Returns <tt>null if there was an error.
     * Allows you to GZip-uncompress the object after it is decoded.
     *
     * @param encodedObject The Base64 data to decode
     * @param gzipCompressed Attempts to decompress decoded object
     * @return The decoded and deserialized object
     * @since 1.5
     */
    public static Object decodeToObject( String encodedObject, boolean gzipCompressed )
    {
        ...
        try
        {
            ...
            ois = gzipCompressed ?
                new java.io.ObjectInputStream( new java.util.zip.GZIPInputStream( bais ) ) :
